...
http_port = options.fetch("http_port", 80).to_i
...
OptionParser.parse do |parser|
  ...
  parser.on("-w PORT", "--web PORT", "Specify HTTP port to use") do |port|
    http_port = port.to_i
  end
...
http_server = HTTPServer.new(store, pop3_port)
